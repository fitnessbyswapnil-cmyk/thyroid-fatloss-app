      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="relative flex-1">
            <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2" style={{ color: "#8b867c" }} />
    ...
    <div className="space-y-3">
      ...
      {editing && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl space-y-3" style={card}>
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold" style={{ color: "#1c1d20" }}>{editing.id ? "Edit exercise" : "New exercise"}</h3>
            <button onClick={() => setEditing(null)} style={{ color: "#8b867c" }} aria-label="Close"><X size={16} /></button>
          </div>
      ...
      {filtered.length === 0 && !editing && (
        <div className="p-10 rounded-2xl text-center" style={card}>
          <Dumbbell size={28} className="mx-auto mb-3" style={{ color: "#cfc7b6" }} />
          <p className="text-sm" style={{ color: "#8b867c" }}>{items.length === 0 ? "No exercises yet — add your first." : "No matches."}</p>
        </div>
      )}
        ...
        <div key={e.id} className="p-4 rounded-2xl flex items-start gap-3" style={card}>
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-semibold" style={{ color: "#1c1d20" }}>{e.name}</span>
              {e.muscle_group && <span className="px-2 py-0.5 rounded text-[10px] uppercase" style={{ background: "rgba(21, 94, 86,0.12)", color: "#155e56" }}>{e.muscle_group}</span>}
              {e.equipment && <span className="px-2 py-0.5 rounded text-[10px] uppercase" style={{ background: "#e2dbcd", color: "#8b867c" }}>{e.equipment}</span>}
              {e.video_url && (
                <a href={e.video_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[11px]" style={{ color: "#155e56" }}>
                  <Video size={12} /> video
                </a>
              )}
            </div>
            {e.cues && <p className="text-xs mt-1" style={{ color: "#8b867c" }}>{e.cues}</p>}
          </div>
  ...
  return (
    <div className="space-y-3">
      ...
      {editing && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="p-5 rounded-2xl space-y-3" style={card}>
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold" style={{ color: "#1c1d20" }}>{editing.id ? "Edit food" : "New food"}</h3>
            <button onClick={() => setEditing(null)} style={{ color: "#8b867c" }} aria-label="Close"><X size={16} /></button>
          </div>
          ...
          <div className="flex items-center gap-4">
            <label className="inline-flex items-center gap-2 text-sm cursor-pointer" style={{ color: "#3c3a34" }}>
              <input type="checkbox" checked={editing.is_veg ?? true} onChange={(e) => setEditing({ ...editing, is_veg: e.target.checked })} style={{ accentColor: "#155e56" }} />
              Vegetarian
            </label>
      ...
      {filtered.length === 0 && !editing && (
        <div className="p-10 rounded-2xl text-center" style={card}>
          <Apple size={28} className="mx-auto mb-3" style={{ color: "#cfc7b6" }} />
          <p className="text-sm" style={{ color: "#8b867c" }}>{items.length === 0 ? "No foods yet — add your first." : "No matches."}</p>
        </div>
      )}
      ...
      {filtered.map((f) => (
        <div key={f.id} className="p-4 rounded-2xl flex items-start justify-between gap-4" style={card}>
          <div className="min-w-0">
            ...
            <p className="text-xs mt-1 tabular-nums" style={{ color: "#8b867c" }}>
              {f.calories != null ? `${f.calories} kcal` : "kcal —"}
              {" · P "}{f.protein ?? "—"}{"g · C "}{f.carbs ?? "—"}{"g · F "}{f.fats ?? "—"}g
              {f.tags ? <span style={{ color: "#a09a8e" }}> · {f.tags}</span> : null}
            </p>
          </div>
          <div className="flex items-center gap-1 shrink-0">
