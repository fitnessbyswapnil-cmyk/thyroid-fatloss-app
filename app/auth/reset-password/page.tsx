  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#fdfbf7" }}>
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(21, 94, 86,0.08) 0%, transparent 70%)", filter: "blur(60px)" }}
      />
      <motion.div
      ...
      >
        ...
        <div className="p-8 rounded-3xl" style={{ background: "#ffffff", border: "1px solid #e2dbcd", backdropFilter: "blur(24px)" }}>
          {done ? (
            <div className="text-center py-6">
              ...
              <p className="text-sm" style={{ color: "#3c3a34" }}>Password updated. Taking you in…</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label htmlFor="password" className="block text-xs font-medium uppercase mb-2" style={{ color: "#8b867c", letterSpacing: "0.08em" }}>
                  New password
                </label>
        ...
        <p className="text-center text-sm mt-6" style={{ color: "#8b867c" }}>
          <Link href="/auth/login" style={{ color: "#155e56" }}>Back to sign in</Link>
        </p>
      </motion.div>
    </div>
  )
